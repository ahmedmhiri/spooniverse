...
import RecipesDetails from "../../data/recipes_details.json";

export default async function RecipePage({ params }) {
  const { contryname, recipe } = params;

  const countryRecipes = RecipesDetails[contryname?.toLowerCase()];
  const recipeDetails = countryRecipes?.find(
    (r) => r.name === recipe?.toLowerCase()
  );

  if (!recipeDetails) {
    console.log(" Not found. contryname:", contryname);
    console.log("recipe:", recipe);
    console.log("Available recipes:", countryRecipes?.map(r => r.name));
